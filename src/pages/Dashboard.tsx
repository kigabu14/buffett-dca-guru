...
const Dashboard = () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        try {
          ...
          // Update stocks with current prices - set to live price or null (no fallback)
  ...
  const marketStats = {
    ...
    avgChange: stockData.length > 0 ? 
      stockData.filter(s => s.changePercent !== null).reduce((sum, s) => sum + (s.changePercent || 0), 0) / stockData.filter(s => s.changePercent !== null).length : 0
  };
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-medium">
                        {YahooFinanceService.formatDisplayPrice(stock.current_price_yahoo, stock.symbol.includes('.BK') ? 'THB' : 'USD')}
                      </span>
                    </div>
                    <RealTimeStockPrice 
                      symbol={stock.symbol} 
                  ...
                  return (
                    <TableRow key={stock.symbol}>
                      ...
                      <TableCell className="text-right font-medium">
                        {YahooFinanceService.formatDisplayPrice(stock.price, stock.currency)}
                      </TableCell>
                      <TableCell className={`text-right ${getPriceChangeColor(stock.change)}`}>
                        {stock.change !== null ? `${stock.change >= 0 ? '+' : ''}${YahooFinanceService.formatNumber(stock.change)}` : '-'}
                      </TableCell>
                      <TableCell className={`text-right ${getPriceChangeColor(stock.changePercent)}`}>
                        {stock.changePercent !== null ? `${stock.changePercent >= 0 ? '+' : ''}${YahooFinanceService.formatNumber(stock.changePercent)}%` : '-'}
                      </TableCell>
                      <TableCell className="text-right">
                        {YahooFinanceService.formatLargeNumber(stock.volume)}
                      </TableCell>
                      <TableCell className="text-right">
                        {YahooFinanceService.formatNumber(stock.pe)}
                      </TableCell>
                      <TableCell className="text-right">
                        {YahooFinanceService.formatDisplayPrice(stock.weekHigh52, stock.currency)}
                      </TableCell>
                      <TableCell className="text-right">
                        {YahooFinanceService.formatDisplayPrice(stock.weekLow52, stock.currency)}
                      </TableCell>
