...
// Note: Numeric fields are nullable - null means no data available from backend
...
export class YahooFinanceService {
  ...
  static async getStock(symbol: string): Promise<StockData> {
    try {
      ...
      if (data?.data && data.data.length > 0) {
        ...
        // Compute change & changePercent only when both values exist and previous_close != 0
        const currentPrice = stockData.current_price || null;
        const previousClose = stockData.previous_close || null;
        const change = (currentPrice !== null && previousClose !== null && previousClose !== 0) 
          ? currentPrice - previousClose 
          : null;
        const changePercent = (currentPrice !== null && previousClose !== null && previousClose !== 0)
          ? ((currentPrice - previousClose) / previousClose) * 100
          : null;
        
        // Preserve backend currency, only fallback to 'THB' if symbol ends with '.BK' AND no currency, else 'USD'
        const currency = stockData.currency || 
          (symbol.endsWith('.BK') ? 'THB' : 'USD');
    ...
    try {
      ...
      if (data?.data && data.data.length > 0) {
        return data.data.map((stockData: any) => {
          ...
          // Preserve backend currency, only fallback to 'THB' if symbol ends with '.BK' AND no currency, else 'USD'
          const currency = stockData.currency || 
            (stockData.symbol.endsWith('.BK') ? 'THB' : 'USD');
  ...
  static formatCurrency(value: number | null | undefined, currency?: string | null): string {
  ...
  static formatNumber(value: number | null | undefined, digits = 2): string {
    if (value == null) return '-';
    return value.toFixed(digits);
  }

  static formatDisplayPrice(value: number | null | undefined, currency?: string | null): string {
    if (value == null) return '-';
